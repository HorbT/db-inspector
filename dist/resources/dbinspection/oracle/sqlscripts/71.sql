SELECT di.owner index_owner,
       ...
       DECODE(di.num_rows, 0, '', round(di.distinct_keys / di.num_rows, 2)) selectivity,
			 (select DIS.STALE_STATS from  cdb_ind_statistics dis
    where di.owner = dis.OWNER AND di.index_name = dis.INDEX_NAME AND
       di.table_name = dis.TABLE_NAME AND di.table_owner = dis.TABLE_OWNER and di.CON_ID=dis.CON_ID AND
       dis.OBJECT_TYPE = 'INDEX' and rownum<=1)  STALE_STATS,
       di.BLEVEL 索引的分支层数,
       di.blevel + 1 索引的高度,
 ...
 WHERE di.index_type != 'LOB'
   ...
   AND exists (SELECT 1 FROM cdb_segments nd　where segment_name = di.index_name AND nd.owner = owner and nd.con_id=di.CON_ID)
 and rownum<=3;